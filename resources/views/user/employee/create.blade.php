    ...
    <script>
        $(document).ready(function() {
            ...
            $('select[name="building_id"]').on('change', function() {
                $.post('{{ route('employee.getSaleManagers', ['RolePrefix' => RolePrefix()]) }}', {
                    _token: $('meta[name="csrf-token"]').attr('content'),
                    project_id: $('.building_id').val(),
                }).done(function(response) {
                    var sale_managers = response.sale_manager;
                    sale_managers.foreach( function(){
                        $('#sale_manager_id').append('<option value="five" selected="selected">Five</option>');
                });
                    return false;
                })
            });
