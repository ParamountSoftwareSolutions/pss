            ...
            <div class="col-sm-12">
              <div class="panel panel-default">
                <div class="panel-body">
                  <form method="POST" action="{{route('accounts.balance_sheet')}}">\
                    @csrf
                    <!-- form_open('accounts/accounts/balance_sheet', array('class' => 'form-inline', 'method' => 'post')) ?> -->
                    <?php
                    $today = date('Y-m-d');
                    ?>
                    <div class="form-group">
                      <label class="" for="from_date"><?php echo display('start_date') ?></label>
                      <input type="date" name="dtpFromDate" class="form-control datepicker" id="from_date" placeholder="<?php echo display('start_date') ?>" value="<?php echo $data['from_date'] ?>">
                    </div>
                    <div class="form-group">
                      <label class="" for="to_date"><?php echo display('end_date') ?></label>
                      <input type="date" name="dtpToDate" class="form-control datepicker" id="to_date" placeholder="<?php echo display('end_date') ?>" value="<?php echo $data['to_date'] ?>">
                    </div>
                ...
                <div class="panel-body" id="printArea">
                  <div class="paddin5ps">
                    ...
                    <table width="100%" class="table_boxnew table-bordered" cellpadding="0" cellspacing="0">
                      ...
                      foreach ($data['fixed_assets'] as $assets) {
                        $total_assets = 0;

                        $head_data = App\Models\Accounts_model::assets_info($assets['HeadName']);

                      ?>
                        <tr>
                          <td align="left" class="paddingleft10px <?php if ($assets['HeadName'] == 'Current Asset' || $assets['HeadName'] == 'Non Current Assets') {
                                                                    echo 'balancesheet_head';
                                                                  } ?>"><?php echo $assets['HeadName']; ?></td>
                        ...
                        foreach ($head_data as $assets_head) {
                          $ass_balance = App\Models\Accounts_model::assets_balance($assets_head['HeadCode'], $from_date, $to_date); ?>
                          <?php if ($assets_head['PHeadName'] == 'Current Asset') {
                            $child_head_current = App\Models\Accounts_model::asset_childs($assets_head['HeadName'], $from_date, $to_date);
                          ...
                          ?>
                            ...
                            <?php foreach ($child_head_current as $cchead) {
                              $cur_ass_balance = App\Models\Accounts_model::assets_balance($cchead['HeadCode'], $from_date, $to_date);
                              $schild_head_current = App\Models\Accounts_model::asset_childs($cchead['HeadName'], $from_date, $to_date);
                            ?>
                              ...
                              <?php if ($cchead['HeadName'] == 'Cash At Bank' || $cchead['HeadName'] == 'Account Receivable' || $cchead['HeadName'] == 'Customer Receivable' || $cchead['HeadName'] == 'Loan Receivable') {
                                foreach ($child_head_current as $scchild) {
                                  $cur_bank_balance = App\Models\Accounts_model::assets_balance($scchild['HeadCode'], $from_date, $to_date);
                              ?>
                      ...
                      foreach ($data['liabilities'] as $liability) {
                        $total_liab = 0;
                        $liab_head_data = App\Models\Accounts_model::liabilities_info($liability['HeadName']);
                      ?>
                        <tr>
                          <td align="left" class="paddingleft10px <?php if ($liability['HeadName'] == 'Current Liabilities' || $liability['HeadName'] == 'Non Current Liabilities') {
                                                                    echo 'balancesheet_head';
                                                                  } ?>"><?php echo $liability['HeadName']; ?></td>
                        ...
                        foreach ($liab_head_data as $liab_head) {
                          ...
                          if ($liab_head['HeadName'] == 'Tax') {
                            $child_liability = App\Models\Accounts_model::liabilities_info_tax($liab_head['HeadName']);
                          } else {
                            $child_liability = App\Models\Accounts_model::liabilities_info($liab_head['HeadName']);
                          }
                        ?>
                          ...
                          foreach ($child_liability as $chliab_head) {
                            $liab_balance = App\Models\Accounts_model::liabilities_balance($chliab_head['HeadCode'], $from_date, $to_date);
                      ...
                      $total_expense = 0;
                      $total_income = 0;
                      foreach ($data['incomes'] as $incomelable) {
                        $income_balance = App\Models\Accounts_model::income_balance($incomelable['HeadCode'], $from_date, $to_date);
                      ?>
                        <tr>
                      ...
                      foreach ($data['expenses'] as $expense) {
                        $expense_balance = App\Models\Accounts_model::income_balance($expense['HeadCode'], $from_date, $to_date);
                      ?>
                        <tr>
