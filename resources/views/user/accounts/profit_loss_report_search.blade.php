...
?>
  ...
  <?php
    ...
    <?php $COAID = $HeadCode;
    if ($check) {
      $oResultAmountPreF = DB::select(DB::raw("SELECT SUM(acc_transaction.Debit)-SUM(acc_transaction.Credit) AS Amount FROM acc_transaction INNER JOIN acc_coa ON acc_transaction.COAID = acc_coa.HeadCode WHERE VDate BETWEEN '$dtpFromDate' AND '$dtpToDate' AND COAID LIKE '$COAID%'"))[0];
    } else {
      $oResultAmountPreF = DB::select(DB::raw("SELECT SUM(acc_transaction.Credit)-SUM(acc_transaction.Debit) AS Amount FROM acc_transaction INNER JOIN acc_coa ON acc_transaction.COAID = acc_coa.HeadCode WHERE acc_transaction.IsAppove = 1 AND VDate BETWEEN '$dtpFromDate' AND '$dtpToDate' AND COAID LIKE '$COAID%'"))[0];
    }
    // $q1 = $CI->db->query($sqlF);
...
@section('title', 'Leads')
@section('style')
<link href="<?php echo asset('public/accounts/assets/js/profit_loss_report_search_script.js') ?>" rel="stylesheet" type="text/css" />
@endsection
@section('content')
                    ...
                    <div id="printArea">
                      <div class="panel-body">
                        <table width="100%" class="table_boxnew" cellpadding="5" cellspacing="0">
                          <tr>
                            <td colspan="2" align="center" class="profit_loss_report_search_font_20"><b><?php echo display('statement_of_comprehensive_income') ?><br /><?php echo display('from') ?> <?php echo $data['dtpFromDate'] ?> <?php echo display('end_date') ?> <?php echo $data['dtpToDate']; ?></b></td>
                          </tr>
                          <tr>
                          ...
                          <?php
                          for ($i = 0; $i < count($data['oResultAsset']); $i++) {
                            $Visited[$i] = false;
                          }

                          AssertCoa("COA", "0", 0, $data['oResultAsset'], $Visited, 0, $data['dtpFromDate'], $data['dtpToDate'], 0);
                          ...
                          <?php
                          for ($i = 0; $i < count($data['oResultLiability']); $i++) {
                            $Visited[$i] = false;
                          }
                          $GLOBALS['TotalLiability'] = 0;
                          AssertCoa("COA", "0", 0, $data['oResultLiability'], $Visited, 0, $data['dtpFromDate'], $data['dtpToDate'], 1);
                          ...
                          <tr>
                            <td align="right" class="profit_loss_report_search_b_right">
                              <h4><?php echo display('pflos') ?> <?php echo $TotalAssetF > $TotalLibilityF ? "(Profit)" : "(Loss)"; ?></h4>
                            </td>
