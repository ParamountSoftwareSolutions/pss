                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <a href="{{ route('farmhouse.block.inventory.create', ['RolePrefix' => RolePrefix(),'farmhouse'=>$farmhouse_id,'block'=>$block_id]) }}" class="btn btn-primary"
                                    ...
                                    <table class="table table-striped" id="table-1">
                                        ...
                                        @forelse($inventories as $data)
                                            <tr>
                                                ...
                                                <td>{{ (!empty($data->premium->name) && $data->premium->name == 'corner') ? "Corner" : '' }}</td>
                                                <td>
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(0px, 26px, 0px); top: 0px; left: 0px; will-change: transform;">
                                                            ...
                                                            <a class="dropdown-item has-icon change_status @if($data->status == 'canceled') d-none @endif" data-value="canceled" data-id="{{$data->id}}">Cancelled</a>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <a href="{{ route('farmhouse.block.inventory.edit', ['RolePrefix' => RolePrefix(),'farmhouse'=>$farmhouse_id,'block' => $block_id ,'inventory' => $data->id]) }}"
                                                       class="btn btn-primary px-1 py-0" title="Edit">
                                                        <i class="fa fa-edit"></i>
                                                    </a>
                                                    <button type="button" data-url="{{ route('farmhouse.block.inventory.destroy',['RolePrefix' => RolePrefix(),'farmhouse'=>$farmhouse_id,'block' => $block_id ,'inventory' => $data->id]) }}" data-token="{{csrf_token()}}" title="Delete" class="btn btn-danger px-1 py-0 deleteBtn">
                                                        <i class="fa fa-trash"></i>
                                                    </button>
                                                    <a href="{{ route('farmhouse.block.inventory.show', ['RolePrefix' => RolePrefix(),'farmhouse'=>$farmhouse_id,'block' => $block_id ,'inventory' => $data->id]) }}"
