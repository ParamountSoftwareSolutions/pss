...
class BuildingInventoryOldController extends Controller
{
    public function index($building_id, $floor_id)
    {
        $country = Country::all();
        $floor = BuildingFloor::findOrFail($floor_id);
        $floor_detail = BuildingInventory::with('payment_plan', 'client')->where(['building_id' => $building_id, 'floor_id' => $floor_id])->get();
    ...
    public function store(Request $request,$panel, $building_id, $floor_id)
    {
        ...
        $floor_detail = new BuildingInventory();
    ...
    {
        $floor_detail = BuildingInventory::where(['id' => $id, 'building_id' => $building_id, 'floor_id' => $floor_id])->first();
        $payment_plan = BuildingPaymentPlan::where('property_admin_id', Helpers::user_admin())->get();
    ...
    public function remove_image(Request $request)
    {
        ...
        $floor_detail_file = BuildingInventoryFile::where(['floor_detail_id' => $request->floor_detail_id, 'id' => $request->id])->first();
    ...
    public function filter(Request $request, $panel,$building_id, $floor_id)
    {
        ...
        $floor = BuildingFloor::findOrFail($floor_id);
        $floor_detail = BuildingInventory::where(['building_id' => $building_id, 'floor_id' => $floor_id]);
    ...
    public function change_status(Request $request)
    {
        ...
        $floor_detail = BuildingInventory::findOrFail($request->id);
    ...
    public function comments($panel,$building_id, $floor_id, $floor_detail_id)
    {
        $building = Building::findOrFail($building_id);
        $floor = BuildingFloor::findOrFail($floor_id);
        $floor_detail = BuildingInventory::findOrFail($floor_detail_id);
